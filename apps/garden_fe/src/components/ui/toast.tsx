...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-2 overflow-hidden rounded-md border p-4 pr-6 shadow-lg transition-all data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-80 data-[state=open]:slide-in-from-top-full",
  {
    variants: {
      variant: {
...
const noopToast = { toasts: [] as Toast[], toast: () => '', dismiss: () => {} };
...
function ToastItem({ toast: t, onDismiss, className, ...props }: ToastItemProps) {
  return (
    <div
      role="alert"
      data-state="open"
      className={cn(toastVariants({ variant: t.variant }), className)}
      {...props}
    >
